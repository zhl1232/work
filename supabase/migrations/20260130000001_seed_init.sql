...
BEGIN
    ...
    (v_discuss_id1, v_user_alice, '我觉得可以多带他们做做实验，比如这个网站上的项目。', now() - interval '4 days'),
    (v_discuss_id1, v_user_bob, '同意楼上，动手是最好的老师。', now() - interval '4 days'),
    (v_discuss_id3, v_user_teacher, '检查一下尾翼是否对称，还有重心是否稳固。', now() - interval '1 day');
    ...
    FOR i IN 1..150 LOOP
        ...
        -- 生成随机内容
        IF i % 5 = 0 THEN
            v_content := '确实如此，非常有道理！ +' || i;
        ELSIF i % 5 = 1 THEN
            v_content := '我也遇到过类似的情况，后来解决了。';
        ELSIF i % 5 = 2 THEN
            v_content := '哈哈，太有意思了！';
        ELSIF i % 5 = 3 THEN
            v_content := '感谢分享，学习了。';
