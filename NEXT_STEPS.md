...
- [ ] **排行榜筛选 (Leaderboard System)**
  - **多维度视图**: 支持 **本周 (Weekly)** / **本月 (Monthly)** / **总榜 (All-time)** 切换。
  - **技术方案**: 对于时间段查询，使用 Postgres `Materialized Views` (物化视图) 缓存统计数据，每小时/每日刷新，避免实时聚合海量数据导致的性能问题。
  - **激励机制**: 展示榜单前三名的专属头像光环。
- [ ] **积分商店系统 (XP Store)**
  - **核心循环**: 用户通过活跃获取 XP -> 消耗 XP 兑换个性化装扮。
  - **商品规划**:
    - 🖼️ **头像框**: 霓虹光环、像素边框、节日限定框。
    - 🎨 **个人主页主题**: 解锁不同的背景色或纹理。
    - 📛 **以昵称特效**: 聊天室或评论区的昵称颜色特效。
