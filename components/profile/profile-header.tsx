    ...
    <>
      ...
      <div className="relative h-28 w-full overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
          ...
          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full bg-white/20 hover:bg-white/30 text-white border-0">
      ...
      <div className="relative px-4 -mt-10 mb-4">
        <div className="flex gap-5 items-start">
          ...
          <div className="relative shrink-0">
            ...
            />
            <div className="absolute bottom-0 right-0 h-5 w-5 rounded-full bg-green-500 border-[2px] border-background" title="在线" />
          </div>
          ...
          <div className="flex flex-col justify-center gap-4 min-w-0 flex-1">
            <div className="flex items-stretch divide-x divide-border">
              <div className="flex flex-1 flex-col items-center justify-center px-4 py-1 min-w-[4rem]">
                <span className="text-lg font-bold text-foreground tabular-nums">{followerCount}</span>
              ...
              <div className="flex flex-1 flex-col items-center justify-center px-4 py-1 min-w-[4rem]">
                <span className="text-lg font-bold text-foreground tabular-nums">{totalLikesReceived}</span>
                <span className="text-xs text-muted-foreground mt-0.5">获赞</span>
              </div>
            </div>
            <EditProfileDialog>
              <Button variant="outline" size="sm" className="rounded-lg border-primary/50 text-primary hover:bg-primary/10 w-full sm:w-fit px-5 font-medium h-9">
                编辑资料
              </Button>
            </EditProfileDialog>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <LevelGuideDialog>
            <button type="button" className="flex items-center gap-1 bg-gradient-to-r from-yellow-400 to-orange-500 text-white px-2 py-0.5 rounded-full text-[10px] font-bold shadow-sm shrink-0 hover:opacity-90 transition-opacity">
          ...
          <BadgeGalleryDialog badges={BADGES} unlockedBadges={unlockedBadges} userBadgeDetails={userBadgeDetails}>
            <div className="flex -space-x-1.5 shrink-0 cursor-pointer hover:opacity-80 transition-opacity">
              {(unlockedBadges.size > 0 ? getBadgesForDisplay(BADGES, unlockedBadges, 3) : BADGES.slice(0, 3)).map((b) => (
        ...
        <div className="mt-3">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-[10px] font-medium text-primary">{currentXP}/{nextLevelXP}</span>
          </div>
